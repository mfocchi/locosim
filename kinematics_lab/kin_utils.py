...
def RNEA(g0,q,qd,qdd):
    ...
    p = np.array([p_01, p_02, p_03, p_04])
    pc = np.array([pc_1, pc_2, pc_3, pc_4])
    ...
    for i in range(n):
        ...
        if i == 0:
            ...
            omega[i] = qd[i]*z[i]
            v[i] = np.cross(omega[i],p_)

            omega_dot[i] = qdd[i]*z[i]
            a[i] = np.cross(omega_dot[i],p_) + 0*np.cross(omega[i],np.cross(omega[i],p_))
            
        else:
            p_ = p[i] - p[i-1]

            omega[i] = omega[i-1] + qd[i]*z[i]
            v[i] = v[i-1] + np.cross(omega[i],p_)

            omega_dot[i] = omega_dot[i-1] + qdd[i]*z[i] + qd[i]*np.cross(omega[i-1],z[i])
            a[i] = a[i-1] + np.cross(omega_dot[i],p_) + np.cross(omega[i],np.cross(omega[i],p_))
